...
asyncio.sleep_ms = sleep_ms  # ANDY
...
class MasterClockInner(EuroPiScript):
    def __init__(self):
        ...
        # machine.freq(250_000_000) # ANDY - this is a getter function, not a setter - so how can this work?
        self.step = 1
        ...
        self.DEBUG = False
        self.ANDY_DEBUG = True
        self.cvs_snapshot = []  # ANDY
    ...
    def clockTrigger(self):
        ...
        if self.DEBUG:
            for i in self.tasks:
                if i != 0:
                    # ANDY need to mock e.g. [<Task>] done: False. state: True. data: None. coro: <generator object 'fred' at 3fff0ae0>
                    # print(f'[{i}] done: {str(i.done())}. state: {str(i.state)}. data: {str(i.data)}. coro: {str(i.coro)}')
                    print(f'    [{i}] done: {str(i.done())}.')
                else:
                    print(f'    [0]')
...
#         mc = MasterClockInner()
#         cv = cv1
#         asyncio.create_task(mc.outputPulse(cv))
#         # asyncio.create_task(blonk())
#         await asyncio.sleep(7)
#         print('done async main')


#     loop = asyncio.get_event_loop() # or asyncio.get_running_loop()

#     # asyncio.run(main()) # <----- not enough to get the some_job() to run
